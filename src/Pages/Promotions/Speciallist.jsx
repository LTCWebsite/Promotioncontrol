...
function Speciallist() {
  ...
  function getValueVIP(e) {
    setGetVIP(e.value);
    console.log("--------", e.value);
    console.log("EEEEE:", getVIP);
    AxiosReq.post(
      `api/Special_Package/QueryByPrmtIdSpecialPK?prmt_id=${e?.value}`,
      { headers: header }
    ).then((res) => {
      if (res?.status === 200) {
        console.log("---===>:", res);
    ...
    AxiosReq.post(
      ...
      if (res.status === 200) {
        ...
        if (res.data.resultCode === 201) {
          ...
          if (count > 0) {
            setEmptyPage(false);
            setdataTable((newValue = res.data?.data));
            setloading(false);
  ...
  const ListPrmtId = () => {
    AxiosReq.get(`api/ListPrmtId`, { headers: header }).then((res) => {
      if (res.status === 200) {
        setListPrmtId(res?.data);
        console.log("ListPrmtId", res);
      }
    });

    // console.log("IDprmId:", idPrmtId);
    AxiosReq.get(
      `api/Special_Package/QueryByPrmtIdSpecialPK?prmt_id=${dataListPrmtId?.code}`,
      { headers: header }
    ).then((res) => {
      if (res.status === 200) {
        setByPrmtId(res);
        console.log("ByPrmtId", res);
      }
    });
  };
  ...
  useEffect(() => {
    ...
    Datapakage();
    ListPrmtId(selectPrmtId);
    // TableListSpecial();
  }, [selectPrmtId, selectCate, seletgroup, getVIP, getNetwork]);

  const options_Speciallist = dataListPrmtId?.map((x) => ({
    value: x.prmtId,
    label: x.prmtId,
  }));
